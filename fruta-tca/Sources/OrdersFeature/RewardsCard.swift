...
public struct RewardsCard: View {
  var totalStamps: Int
  ...
  public init(
    totalStamps: Int,
    animatedStamps: Int = 0,
    hasAccount: Bool,
    compact: Bool = false
  ) {
    self.totalStamps = totalStamps
    self.animatedStamps = animatedStamps
  ...
  public var body: some View {
    VStack {
      ...
      .accessibility(label: Text(
        "\(totalStamps) of 10 points earned",
        comment: "Accessibility label for number of points earned"
      ))
      ...
      if !compact {
        Group {
          if hasAccount {
            Text(
              "You are \(10 - totalStamps) points away from a free smoothie!",
              comment: "Label showing the number of points needed to get a free smoothie in rewards card view"
            )
          } else {
            Text(
              "Sign up to get rewards!",
              comment: "Label shown in rewards card view when no account has been created yet"
            )
          }
        }
        .font(Font.system(.headline, design: .rounded).bold())
        .multilineTextAlignment(.center)
        .foregroundStyle(Color.rewardsForeground)
...
extension RewardsCard {
  struct StampSlot: View {
    ...
    var body: some View {
      ZStack {
        Circle().fill(Color.bubblesBackground.opacity(0.5))
      ...
      .onAppear {
        ...
        case .stamped:
          stamped = true
        case let .stampedAnimated(delayIndex):
